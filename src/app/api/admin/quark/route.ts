...
// 夸克网盘中转（list 列文件 / download 转存COS / health 检查cookie）
export const maxDuration = 60;

export async function POST(req: Request) {
  try {
    const { action, pdir_fid, fids } = await req.json();
    if (!action) return NextResponse.json({ error: "缺少 action" }, { status: 400 });
    const result = await invokeCloudFunction("quark", { action, pdir_fid: pdir_fid || "0", fids: fids || [] });
    if (result.code !== 0) {
